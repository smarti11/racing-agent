"""Publish dashboard/racing.html to S3 after each regen.

Uses the AWS CLI (`aws s3 cp`) so the Mac Mini can rely on its existing
`aws configure` credentials — no boto3 dependency required.
"""
...
def publish_dashboard_to_s3(
    ...
    *,
    ...
    if not enabled:
        logger.debug("S3 dashboard publish disabled")
        return False
